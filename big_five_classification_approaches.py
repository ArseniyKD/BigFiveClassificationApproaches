...
def KNNClassifierWithTuning( X_train, y_train, X_val, y_val, n_neighbors=None, leaf_size=None, p=None ):
    ...
    if n_neighbors is not None:
        ...
        neighs = [1] + list( range( 10 : 50 : 10 ) )
    
    leafs = None
    if leaf_size is not None:
        leafs = leaf_size
    else:
        leafs = [1] + list( range( 5 : 30 : 5 ) )
    ...
    for p in ps:
        knn = neighbors.KNeighborsClassifier( p=p )
    ...
    model = neighbots.KNeighborsClassifier( n_neighbots=best_n_neighbors,
...
def main():
    ...
    KNNModel, KNNModel_val_acc, KNN_acc_train, KNN_best_n_neighbors, KNN_best_leaf_size, KNN_best_p = KNNClassifierWithTuning( X_train, y_train, X_val, y_val )
    #print( "Validation accuracy:", KNN_acc*100 )
    #print( "Test accuracy:", KNNModel.score( X_test, y_test )*100 )
    print( "KNN Model validation accuracy:", str(KNNModel_acc*100) + "%", "Scoring test accuracy." )
